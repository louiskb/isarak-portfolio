RubyLLM.configure do |config|
  ...
  #
  # If just using own personal OpenAI API key for development and testing then can skip `!!!! ...comments... !!!!`.
  config.openai_api_key = ENV.fetch("OPENAI_API_KEY", nil)

  # !!!! IF TESTING IN DEVELOPMENT: Uncomment code below if testing AI Blog Features in development. Also uncomment `GITHUB_TOKEN` and comment out OPENAI_API_KEY` in `.env` file. (if it doesn't work check that there's still free credits on the GitHub API key and it hasn't expired) !!!!
  #
