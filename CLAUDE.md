...
- **Search + tag filters on all resource indexes** — Research, Teaching, Grants & Awards, and Blog each have a title search (`q`, `ILIKE`) + multi-select tag-filter pills (`tag_ids[]`). The blog markup/styles are reused via `shared/_resource_filter` (+ `blog_filter_controller`). Isara creates/deletes tags inline on each new/edit form via `shared/_tag_manager` (+ `tag_manager_controller`, POST/DELETE `/tags`). **Each resource owns an isolated tag library** — the `tags` table has a `resource_type` column (`blog_post`, `research_item`, `teaching`, `grant_award`) and controllers scope pills via `Tag.for_resource(<type>)`. A tag made on the blog is **not** visible on research/teaching/awards, and deleting a tag only affects that one resource. Names are unique **per `resource_type`** (composite unique index `[resource_type, name]`), so the same name (e.g. `Housing`) can exist independently in each section. Tag creation POSTs a `resource_type` alongside the name (from `tag_manager_controller`'s `resourceParamValue`). Join tables are unchanged (`blog_post_tags`, `research_item_tags`, `teaching_tags`, `grant_award_tags`; `Tag` keeps reciprocal `dependent: :destroy`, but a tag now has rows only in its own resource's join table). While a search/filter is active on a drag-reorderable index, drag handles + SortableJS are disabled (`@filtering`) so a filtered subset can't corrupt `position`.
