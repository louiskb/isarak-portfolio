...
- [x] PostHog analytics — cookieless visitor tracking (no cookies, no cookie banner needed) ✅ (2026-04-07)
  - [x] Switched to real `cookieless_mode: 'always'` and moved to **EU Cloud** ✅ (2026-08-25). Before this it was cookieless only by side effect (`persistence: 'memory'`), so the id reset every page load and returning visitors counted as new people. ⚠️ The old "no cross-session tracking" claim was falsified by the switch and was removed from the privacy policy and three docs.
